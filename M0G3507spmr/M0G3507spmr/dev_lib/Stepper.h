...
struct Steeper_t *Stepper_Init(UART_Regs *pUSARTx, uint8_t address, struct Buff *BUFF, enum Stepper_Check_Way_t check_way, enum Stepper_FOC_Version_t FOC_VERSION);
...
struct Steeper_t
{
	...
	struct Steeper_t *(*Init)(UART_Regs *pUSARTx, uint8_t address, struct Buff *BUFF, enum Stepper_Check_Way_t check_way, enum Stepper_FOC_Version_t FOC_VERSION);
...
void Stepper_synchronization(UART_Regs * USART);
