...
void Stepper_synchronization(UART_Regs * pUSARTx)
{
	uint8_t data[4];
	data[0] = 0x00;
	data[1] = 0xFF;
...
//		App_Printf("Stepper_Read_Current_Position: check way error!\n");
	...
	delay_ms(3); // wait for the response
...
struct Steeper_t *Stepper_Init(UART_Regs *pUSARTx, uint8_t address, struct Buff *BUFF, enum Stepper_Check_Way_t check_way, enum Stepper_FOC_Version_t FOC_VERSION)
{
	...
	this = (struct Steeper_t *)malloc(sizeof(struct Steeper_t));
	if (this == NULL)
	{
